...
public class HierCtx implements Context {
	...
	protected NameParser myParser = new Rfc2253NameParser();
	...
	public void bind(Name name, Object obj) throws NamingException {
		...
		if (nm.size() == 1) {
			...
			obj = NamingManager.getStateToBind(obj, new CompositeName()
					.add(atom), this, myEnv);
	...
	public Context createSubcontext(Name name) throws NamingException {
		...
		if (nm.size() == 1) {
			...
			if (null == inter) {
				...
				// TODO set the remaining and resolved name correctly
				nameNotFoundException.setRemainingName(name);
				String resolvedName = myAtomicName;
				resolvedName = (myAtomicName == null) ? "" : myAtomicName;
			...
			if (!(inter instanceof Context)) {
				final NotContextException notContextException = new NotContextException(
						"\"" + atom + "\" does not name a context");
				// TODO set the remaining and resolved name correctly
				notContextException.setRemainingName(new CompositeName(""));
				notContextException.setResolvedName(new CompositeName(""));
	...
	class ListOfBindings extends ListOfNames {
		...
		public Object next() throws NamingException {
			...
			try {
				obj = NamingManager.getObjectInstance(obj, new CompositeName()
						.add(name), HierCtx.this, HierCtx.this.myEnv);
			} catch (Exception e) {
				NamingException ne = new NamingException(
						"getObjectInstance failed");
				ne.setRootCause(e);
